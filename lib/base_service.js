...
class BaseService extends Egg.Service {
  ...
  constructor(ctx, model, preset = {}) {
    ...
    if (this.operatorLog) {
      // 自动生成日志相关数据预处理
      this.fieldNameMap = {};
      _.map(this.model.rawAttributes, (rawAttrInfo, rawAttrKey) => {
 ...
 //  * @param {string} [options.deferrable]   可选，设置立即或延迟检查约束 （貌似仅用于 postgresql）
   ...
   * @param {Object}  [options={}]
   * @param {boolean} [options.plain=false]
   * @param {boolean} [options.fake=false]          -  可选，是否为假删除， 默认 false
   * @param {string}  [options.fakeBody]      -  可选，fake 为 true 时有效，假删除时候更新的数据（键值对），如果存在此值，则 fakeField 无效
   * @param {string}  [options.fakeField]     -  可选，fake 为 true 并且无 fakeBody 时有效，假删除标记用的·字段·（默认为 deleteTime），字段类型为时间
   * @param {boolean} [options.retValues=false]     -  可选，是否返回相关实例，默认 false
   * @param {Object}  [options.transaction]   -  可选，使用事务.
   * @param {Object}  [options.lock]          -  可选，使用事务时.锁定已选行. 取值有 transaction.LOCK.UPDATE、 transaction.LOCK.SHARE （必须通过事务对象实例获取）
   * @returns {Object}
  ...
  async remove({ params = {} }, options = {}) {
    ...
    if (options.fake) {
      ...
      if (options.fakeBody && _.keys(options.fakeBody).length >= 0) {
        ...
        body = { [options.fakeField]: Date.now() };
      }
