...
class BaseService extends Service {
 ...
 //  * @param {String} [options.deferrable]   可选，设置立即或延迟检查约束 （貌似仅用于 postgresql）
   ...
   * @memberof BaseService
   */
  async listAll({ params = {}, query = {} }, options = {}) {
  ...
  async count({ params = {}, query = {} }, options = {}) {
    options.count = true;
    options.list = false;
    return this.listAll({ params, query }, options);
  }
  ...
  async set({ params = {} }, records, options = {}) {
    ...
    const toEdit = [];
    const toRemove = [];
    const oldVals = await this.listAll({ params }, options);
    ...
    if (options.retValues) {
      const newVals = await this.listAll({ params }, options);
  ...
  async moreById(id, { params = {}, query = {} }, options = {}) {
    ...
    // }
    if (idx >= 0) {
      const datas = await this.listAll({ params, query }, options);
      return {
        datas,
        next: true,
      };
  ...
  async nth({ params = {} }, index, options = {}) {
    return this.listAll({ params, query: { page: index, size: 1, offset: 0 } }, options);
  }
  ...
  async first({ params = {} }, options = {}) {
    return this.listAll({ params, query: { page: 1, size: 1, offset: 0 } }, options);
  }
  ...
  async indexById(id, { params = {}, query = {} }, options = {}) {
    ...
    options.maxSize = Number.isInteger(Number(options.maxSize)) && Number(options.maxSize) >= 0 ? ~~options.maxSize : 1000;
    const list = await this.listAll({ params, query: { size: options.maxSize, offset } },
