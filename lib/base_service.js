...
class BaseService extends Egg.Service {
   ...
   * @param {Object}   [preset.enums]           -  字段可取值枚举，如 { enable: { 0: '禁用', 1: '启用' } } 。 仅 operatorLog === true 有用。推荐使用 config.enums[model]
   * @param {Object}   [preset.enumsInvert]     -  字段可取值枚举(逆))，如 { enable: { '禁用': 0, '启用': 1 } } 。仅 operatorLog === true 有用。推荐使用 config.enumsInvert[model]
  ...
  constructor(ctx, model, preset = {}) {
    ...
    if (this.operatorLog) {
      ...
      this.enums = preset.enums || {};
      this.enumsInvert = preset.enumsInvert || {};
 ...
 //  * @param {string} [options.deferrable]   可选，设置立即或延迟检查约束 （貌似仅用于 postgresql）
  ...
  async list({ params = {}, query = {} }, options = {}) {
    ...
    this[HANDLESCOPE](scope, where, query);
    const findOptions = {
  ...
  async edit({ params = {}, body = {} }, options = {}) {
    ...
    if (options.retDiffs || this.operatorLog) {
    ...
    if (options.retValues || this.operatorLog) {
      ...
      if ((oldInstances && oldInstances.length) || this.operatorLog) {
  ...
  async editById(id, values, options = {}) {
    ...
    if (options.retValue || this.operatorLog) {
      ...
      if (oldInstance || this.operatorLog) {
   ...
   * @param {Egg.Scope} [options.scope='default']
   * @param {boolean} [options.plain=false]
   * @param {Object}  [options.attributes]      -  可选，功能同 Sequelize.increment 方法的 attributes 参数，用于更新数据，但不需要自增等才做，如传入 { deleteUserId: 3 }, 则该操作同时也会设置 deleteUserId 值为 3
   * @param {boolean} [options.retValues=false]     -  可选，是否返回相关实例，默认 false
   * @param {boolean} [options.retDiffs=false]      -  可选，是否返回记录的调整情况，默认 false
   * @param {Object}  [options.transaction]   -  可选，使用事务.
   * @param {Object}  [options.lock]          -  可选，使用事务时.锁定已选行. 取值有 transaction.LOCK.UPDATE、 transaction.LOCK.SHARE （必须通过事务对象实例获取）
  ...
  async editSelf({ params = {}, body = {} }, method, options = {}) {
    ...
    if (options.retValues || this.operatorLog) {
      ...
      if ((oldInstances && options.retDiffs) || this.operatorLog) {
        oldInstances = BaseService.toJSON(oldInstances, true);
        const newInstances = BaseService.toJSON(instances, true);
        const diffs = BaseService.diffObjects(oldInstances, newInstances);
        ret = {
  ...
  async editSelfById(id, values, method, options = {}) {
    ...
    if (oldInstance) {
      ...
      if (options.retValue || this.operatorLog) {
        const instance = await this.model.findOne({ where, transaction: options.transaction, lock: options.lock });
        if ((oldInstance && options.retDiff) || this.operatorLog) {
          oldInstance = BaseService.toJSON(oldInstance, true);
          const newInstance = BaseService.toJSON(instance, true);
          const diff = BaseService.diffObject(oldInstance, newInstance);
          ret = {
      ...
      if (matchInclude && association) {
        if (association.associationType === 'BelongsToMany') {
          ...
          const datas = BaseService.toJSON(await matchInclude.model.findAll({
            attributes: [],
            where     : { ...matchInclude.where, [whereKey]: value },
  ...
  [SETXXXUSERANDTIMERAW](type, body) {
    ...
    if (this[timeField] && !body[this[timeField]]) {
      body[this[timeField]] = this.model._getDefaultTimestamp(this[timeField]) || Utils.now(this.sequelize.options.dialect);
    }
  ...
  [GETUPDATEEDATA](where, query, modelInfo) {
    ...
    if (info.updateTimeField && !query[info.updateTimeField]) {
      ...
      } else if (attr.type instanceof Sequelize.DataTypes.NUMBER || attr.type instanceof Sequelize.DataTypes.DATE || attr.type instanceof Sequelize.DataTypes.TIME) {
   ...
   * @memberof BaseService
   */
  [GETDELETEEDATA](where, query, modelInfo) {
    const info = modelInfo || this;
    // 只要操作时间不为 null 即可，操作人还是可能为 null 的
    // if (info.deleteUserIdField && !query[info.deleteUserIdField]) {
    //   where[info.deleteUserIdField] = { [Op.not]: null };
    // }
    if (info.deleteTimeField && !query[info.deleteTimeField]) {
   ...
   *                             为其字面意思，分别为：已经删除的（假删除），被编辑过的（已过滤假删除数据），所有的
   * @param {Object} where where 条件对象
   * @param {Object} query  query数据
   * @param {string|Egg.IModel} [model] 可选，默认为空，使用当前 model 相应数据，用于联表的 操作人、时间 字段的信息获取（来源于其他 service）
   * @memberof BaseService
   */
  [HANDLESCOPE](scope, where, query, model) {
    let modelInfo;
    if (model) {
    ...
    switch (scope) {
      case 'deleted':
        this[GETDELETEEDATA](where, query, modelInfo);
        break;
      case 'updated':
        this[GETDELETEEDATA](where, query, modelInfo);
        this[GETUPDATEEDATA](where, query, modelInfo);
        break;
      case 'all':
        // pass 不需要追加条件
        break;
  ...
  [DEALOPERATORLOG](obj) {
    ...
    _.map(obj.logInfos, logInfo => {
      ...
      if (logInfo.type === 'add') {
        ...
        message.op = '查询';
        message.count = logInfo.count;
        if (logInfo.method.indexOf('list') >= 0) {
          message.content = `${message.op}列表 ( ${message.count} 条记录 ) `;
        } else {
          message.content = `${message.op}记录`;
        }
        message.include = [];
        if (logInfo.include) {
          _.map(logInfo.include, (val, key) => {
        ...
        if (logInfo.params) {
          _.map(logInfo.params, (val, key) => {
            message.condition.push(`${fieldNameMap[key] || key}：${symbolObjectStringify(val)}`);
          });
        }
...
_.map(funcNames, func => {
  ...
  BaseService.prototype[func] = async function() {
    ...
    return await tmp.bind(this)(...arguments);
  };
});
