...
class BaseController extends Egg.Controller {
  ...
  constructor(ctx, service, preset = {}) {
    ...
    // findOpts
    _.assign(this, {
      param     : preset.param || this.model.primaryKeyAttribute || 'id',
      size      : preset.size || 20,
      rules     : preset.rules || {},
  ...
  async remove() {
    ...
    // const id = ctx.params[this.param];
    const params = ctx.params;
    const { fake, deleteUserIdField, deleteTimeField } = this.model;
    const fakeBody = {};
    if (fake) {
    ...
    if (include) {
      ...
      _.map(include, item => {
        if ((!this.limit.include || (this.limit.include.indexOf(item) >= 0))
          ...
          includeList.push({ preset: item, attributes: attributes || undefined });
        }
      });
    }
    return includeList;
