...
class BaseService extends Egg.Service {
 ...
 //  * @param {string} [options.deferrable]   可选，设置立即或延迟检查约束 （貌似仅用于 postgresql）
  ...
  async list(args = {}, options = {}) {
    ...
    let result;
    // find and count
    const countOptions = _.cloneDeep(findOptions);
    ...
    if (options.count) {
      if (options.list) {
        ...
        let rows = [];
        if (count) {
          rows = await this.model.findAll(findOptions);
          rows = BaseService.toJSON(rows, options.plain);
        }
        result = {
          count,
          rows,
        ...
        result = await this.model.count(countOptions);
